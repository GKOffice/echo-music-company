...
@router.post("/migrate-deal-room")
async def migrate_deal_room(
    current_user: TokenData = Depends(require_admin),
    db: AsyncSession = Depends(get_db),
):
    """Create deal_listings and deal_offers tables (one-time migration)."""
    try:
        ...
        """))
        await db.execute(text("CREATE INDEX IF NOT EXISTS idx_deal_listings_creator ON deal_listings(creator_id)"))
        await db.execute(text("CREATE INDEX IF NOT EXISTS idx_deal_listings_status ON deal_listings(status)"))
        await db.execute(text("CREATE INDEX IF NOT EXISTS idx_deal_offers_listing ON deal_offers(listing_id)"))
        await db.execute(text("CREATE INDEX IF NOT EXISTS idx_deal_offers_buyer ON deal_offers(buyer_id)"))
        await db.commit()
        return {"status": "complete", "tables": ["deal_listings", "deal_offers"]}
